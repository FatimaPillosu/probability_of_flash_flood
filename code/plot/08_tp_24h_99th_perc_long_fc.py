...
###################################################################################
# CODE DESCRIPTION
# 08_tp_24h_99th_perc_long_fc.py plots the 99th percentile of 24-hourly long-range rainfall 
# forecasts from ERA5-ecPoint.

# Usage: python3 08_tp_24h_99th_perc_long_fc.py
...
###################################################################################
# INPUT PARAMETERS
base_date = datetime(2021,8,29, 0)
step_f = 96
mask_domain = [22,-130,52,-60]
git_repo = "/ec/vol/ecpoint_dev/mofp/phd/probability_of_flash_flood"
file_in_mask = "data/raw/mask/usa_era5.grib"
dir_in = "data/raw/reanalysis/era5_ecpoint/tp_24h_long_fc"
dir_out = "data/plot/08_tp_24h_99th_perc_long_fc"
###################################################################################
...
# Plotting the 99th percentile for ERA5_ecPoint's 24-hourly long-range rainfall forecasts
print()
print("Plotting the 99th percentile for ERA5_ecPoint's 24-hourly long-range rainfall forecast for:")
print(f'{base_date.strftime("%Y-%m-%d")} at {base_date.strftime("%H")} UTC (t+{step_s}, t+{step_f})')

tp = mv.read(f'{git_repo}/{dir_in}/{base_date.strftime("%Y%m")}/Pt_BC_PERC_{base_date.strftime("%Y%m%d")}_{step_f:03d}.grib2')
tp_perc = tp[perc-1]
...
title_plot1 = f'ERA5-ecPoint {acc}-hourly tp (mm/{acc} h), {perc}th percentile'
...
file_out = f'{dir_out_temp}/tp_{acc}h_{perc}th_{base_date.strftime("%Y%m%d")}_{base_date.strftime("%H")}_{step_f:03d}'
