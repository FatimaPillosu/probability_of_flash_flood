...
###########################################################################################################
# CODE DESCRIPTION
# 06_swvl_1m_short_fc.py computes the instantaneous percentage of ERA5's short-range soil saturation (swvl) for the top 1m level, 
# at 00 UTC.

# Usage: python3 06_swvl_1m_short_fc.py 2024
...
dir_in = "data/raw/reanalysis/era5/swvl_short_fc"
dir_out = "data/processed/06_swvl_1m_short_fc"
###########################################################################################################
...
print("Computing the instantaneous percentage of soil saturation:")
the_date = the_date_s
while the_date <= the_date_f:

      print(" - on " + the_date.strftime("%Y-%m-%d") + " at " + the_date.strftime("%H") + " UTC")
      
      # Read the volumetric soil water for level 1 (0 - 7 cm)
      swvl = "swvl1"
      FileIN_1 = git_repo + "/" + dir_in + "/" + swvl + "/" + the_date.strftime("%Y") + "/" + the_date.strftime("%Y%m%d") + "/" + swvl + "_" + the_date.strftime("%Y%m%d") + "_" + the_date.strftime("%H") + ".grib"
      swvl1 = mv.read(FileIN_1)
     
      # Read the volumetric soil water for level 2 (7 - 28 cm)
      swvl = "swvl2"
      FileIN_2 = git_repo + "/" + dir_in + "/" + swvl + "/" + the_date.strftime("%Y") + "/" + the_date.strftime("%Y%m%d") + "/" + swvl + "_" + the_date.strftime("%Y%m%d") + "_" + the_date.strftime("%H") + ".grib"
      swvl2 = mv.read(FileIN_2)
      
      # Read the volumetric soil water for level 3 (28 - 100 cm)
      swvl = "swvl3"
      FileIN_3 = git_repo + "/" + dir_in + "/" + swvl + "/" + the_date.strftime("%Y") + "/" + the_date.strftime("%Y%m%d") + "/" + swvl + "_" + the_date.strftime("%Y%m%d") + "_" + the_date.strftime("%H") + ".grib"
      ...
      # Save the field
      dir_out_temp = git_repo + "/" + dir_out + "/" + the_date.strftime("%Y")
      if not os.path.exists(dir_out_temp):
            os.makedirs(dir_out_temp)
      file_out = dir_out_temp + "/swvl_1m_perc_" + the_date.strftime("%Y%m%d%H") + ".grib"
