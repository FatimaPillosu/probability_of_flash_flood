...
################################################################################################
# CODE DESCRIPTION
# 10_prob_ff_hydro_short_fc_pdt_year.py computes the training dataset (Point Data Table) for a specific year to 
# compute the short-range data-driven forecasts of areas at risk of flash floods.

# Usage: python3 10_prob_ff_hydro_short_fc_pdt_year.py 2024
...
dir_out = "data/processed/10_prob_ff_hydro_short_fc_pdt_year"
################################################################################################
...
print(" - Reading the slor and sdfor values within the considered domain")
...
tp_prob_2_all = np.array([])
tp_prob_5_all = np.array([])
tp_prob_10_all = np.array([])
tp_prob_20_all = np.array([])
tp_prob_50_all = np.array([])
tp_prob_100_all = np.array([])
...
while the_date_start <= the_date_start_f:
      ...
      print(" - Reading the predictand (flash flood reports) and the predictors (e.g. rainfall probabilities, swvl, slor, sdfor, and lai) for " + the_date_start.strftime("%Y%m%d"))
      
      # Reading the flash flood reports
      file_in_ff = git_repo + "/" + dir_in_ff + "/" + the_date_final.strftime("%Y") + "/grid_acc_reports_ff_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H")  + ".grib"
      ...
      if os.path.exists(file_in_ff): # building the point data table only for those days with some flash flood reports in the domain
            ...
            file_in_tp_prob_2 = git_repo + "/" + dir_in_tp_prob + "/" + str(2) + "rp/" + the_date_final.strftime("%Y%m") + "/prob_exceed_rp_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H") + ".grib"
            tp_prob_2 = mv.values(mv.read(file_in_tp_prob_2))[ind_mask]

            file_in_tp_prob_5 = git_repo + "/" + dir_in_tp_prob + "/" + str(5) + "rp/" + the_date_final.strftime("%Y%m") + "/prob_exceed_rp_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H") + ".grib"
            tp_prob_5 = mv.values(mv.read(file_in_tp_prob_5))[ind_mask]

            file_in_tp_prob_10 = git_repo + "/" + dir_in_tp_prob + "/" + str(10) + "rp/" + the_date_final.strftime("%Y%m") + "/prob_exceed_rp_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H") + ".grib"
            tp_prob_10 = mv.values(mv.read(file_in_tp_prob_10))[ind_mask]

            file_in_tp_prob_20 = git_repo + "/" + dir_in_tp_prob + "/" + str(20) + "rp/" + the_date_final.strftime("%Y%m") + "/prob_exceed_rp_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H") + ".grib"
            tp_prob_20 = mv.values(mv.read(file_in_tp_prob_20))[ind_mask]
            ...
            file_in_tp_prob_100 = git_repo + "/" + dir_in_tp_prob + "/" + str(100) + "rp/" + the_date_final.strftime("%Y%m") + "/prob_exceed_rp_" + the_date_final.strftime("%Y%m%d") + "_" + the_date_final.strftime("%H") + ".grib"
            tp_prob_100 = mv.values(mv.read(file_in_tp_prob_100))[ind_mask]
            ...
            dates_all = np.concatenate((dates_all, np.array([the_date_final.strftime("%Y%m%d%H")] * lats.shape[0])))
            ...
            tp_prob_2_all = np.concatenate((tp_prob_2_all, tp_prob_2))
            tp_prob_5_all = np.concatenate((tp_prob_5_all, tp_prob_5))
            tp_prob_10_all = np.concatenate((tp_prob_10_all, tp_prob_10))
            tp_prob_20_all = np.concatenate((tp_prob_20_all, tp_prob_20))
            ...
            tp_prob_100_all = np.concatenate((tp_prob_100_all, tp_prob_100))
...
pdt = pd.DataFrame()
pdt["date_time_end_24h"] = dates_all
...
pdt["tp_prob_2"] = tp_prob_2_all.astype(float).round(2)
pdt["tp_prob_5"] = tp_prob_5_all.astype(float).round(2)
pdt["tp_prob_10"] = tp_prob_10_all.astype(float).round(2)
pdt["tp_prob_20"] = tp_prob_20_all.astype(float).round(2)
...
pdt["tp_prob_100"] = tp_prob_100_all.astype(float).round(2)
...
pdt.to_csv(dir_out_temp + "/pdt_" + str(year) + ".csv", index=False)